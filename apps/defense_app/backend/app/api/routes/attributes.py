...
@router.get("/{attr_id:path}/heat-series", response_model=HeatSeriesResponse)
...
@router.get("/{attr_id:path}/articles", response_model=AttributeArticlesResponse)
...
@router.get("/{attr_id:path}/graph", response_model=AttributeGraphResponse)
...
@router.get("/{attr_id:path}", response_model=AttributeDetailResponse)
def get_attribute(
    attr_id: str,
    connection: Annotated[sqlite3.Connection, Depends(get_database)],
    source_week: int | None = None,
