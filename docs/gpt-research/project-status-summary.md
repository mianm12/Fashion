...
| 优先级 | 任务 | 说明 |
| --- | --- | --- |
| 高 | 导出论文表格 | 把趋势模型、推荐模型、数据规模、时间切分整理成论文表 |
| 高 | 导出核心图 | 流程图、特征重要性、趋势指标对比、推荐指标对比 |
| 高 | 扩展推荐解释案例 | 当前已有 1 个案例；论文可扩到 2-3 个 |
| 中 | Top-K 趋势属性案例 | 已补文本表；后续可导出为论文图 |
...
| 趋势属性可视化图 | 已有文本榜单和 8 周表格 | 需要导出折线图/柱状图 |
| 推荐解释案例扩展 | 已补 1 个命中案例 | 可继续挑选 1-2 个用户丰富论文展示 |
...
| 后续图表尚未导出 | 不影响开始论文正文，但提交前需要整理成最终图表 |
